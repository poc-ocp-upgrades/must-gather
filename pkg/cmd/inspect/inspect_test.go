...
func TestDirectoryViable(t *testing.T) {
	...
	tc := []struct {
		...
		allowOverride	bool
		expectedErr	error
	}{{name: "ensure non-existent directory is viable", dirName: func() (string, error) {
		return "/foo/bar/baz", nil
	}}, {name: "ensure empty directory is viable", dirName: func() (string, error) {
		tmpDir, err := ioutil.TempDir(os.TempDir(), "must-gather-inspect-")
		if err != nil {
			return "", err
		}
		return tmpDir, nil
	}, teardown: defaultTempDirTeardown}, {name: "ensure non-empty directory not viable", dirName: func() (string, error) {
		tmpDir, err := ioutil.TempDir(os.TempDir(), "must-gather-inspect-")
		if err != nil {
			return "", err
		}
		...
		return tmpDir, err
	}, expectedErr: fmt.Errorf("exists and is not empty"), teardown: defaultTempDirTeardown}, {name: "ensure non-empty directory viable with data override", dirName: func() (string, error) {
		tmpDir, err := ioutil.TempDir(os.TempDir(), "must-gather-inspect-")
		if err != nil {
			return "", err
		}
		_, err = ioutil.TempFile(tmpDir, "must-gather-inspect-file-")
		return tmpDir, err
	}, allowOverride: true, teardown: defaultTempDirTeardown}}
	for _, test := range tc {
...
func TestAPIGroupVersionRetrieval(t *testing.T) {
	...
	tc := []struct {
		...
		expectedErr		error
	}{{name: "ensure unknown apiGroup returns no results", resourceFinder: &fakeSupportedResourceFinder{supportedResources: fakeAPIResourceList()}, apiGroup: "unknown"}, {name: "ensure known apiGroup returns expected results", resourceFinder: &fakeSupportedResourceFinder{supportedResources: fakeAPIResourceList()}, apiGroup: "test.group.io", expectedResources: []schema.GroupVersionResource{{Group: "test.group.io", Version: "v1", Resource: "testresources"}}}, {name: "ensure different, but known apiGroup returns expected results", resourceFinder: &fakeSupportedResourceFinder{supportedResources: fakeAPIResourceList()}, apiGroup: "foo.group.io", expectedResources: []schema.GroupVersionResource{{Group: "foo.group.io", Version: "v1", Resource: "foos"}}}, {name: "ensure known apiGroup with NO `list` verb is omitted from results", resourceFinder: &fakeSupportedResourceFinder{supportedResources: []*metav1.APIResourceList{{TypeMeta: metav1.TypeMeta{Kind: "test", APIVersion: "v1"}, GroupVersion: schema.GroupVersion{Group: "test.group.io", Version: "v1"}.String(), APIResources: []metav1.APIResource{{Name: "testresources", SingularName: "testresource", Group: "test.group.io", Version: "v1", Kind: "test", Verbs: []string{"get"}}}}}}, apiGroup: "test.group.io"}, {name: "ensure only discovery errors are returned when no resources are found", resourceFinder: &fakeSupportedResourceFinder{withError: fmt.Errorf("test")}, apiGroup: "foo", expectedErr: fmt.Errorf("test")}}
	for _, test := range tc {
		t.Run(test.name, func(t *testing.T) {

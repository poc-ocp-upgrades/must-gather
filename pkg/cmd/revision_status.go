...
func NewCmdRevisionStatus(parentName string, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "revision-status -n <namespace>", Short: "Counts failed installer pods and current revision of static pods.", Example: fmt.Sprintf(revisionStatusExample, parentName), SilenceUsage: true, RunE: func(c *cobra.Command, args []string) error {
...
func (o *RevisionStatusOptions) Run() error {
	...
	revisions := []struct {
		name	string
		ids		[]string
	}{{name: "Succeeded", ids: succeededRevisionIDs}, {name: "Failed", ids: failedRevisionIDs}, {name: "InProgress", ids: inProgressRevisionIDs}, {name: "Unknown", ids: unknownStatusRevisionIDs}}
